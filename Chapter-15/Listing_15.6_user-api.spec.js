...
describe("the UserApi service", () => {
  ...
  it("loads user's country", done => {
    httpClient.fetch.and.returnValue(
      TestHelper.mockResponseUnwrapped(countries)
    );
    sut
      .loadCountry("au")
  ...
  it("loads an empty country as a fallback", done => {
    httpClient.fetch.and.returnValue(TestHelper.mockResponseUnwrapped([]));
    sut
      .loadCountry()
  ...
  it("gets all users", done => {
    httpClient.fetch.and.returnValue(TestHelper.mockResponseUnwrapped(users));
    sut
      .getUsers()
